...
// константы преобразований для датчика 300 PSI с внешним 24-битным АЦП (VREF =
// 5V)
...
constexpr uint32_t ADC_MAX_24BIT = 8388607u;

// диапазон измерения: 0...300 PSI
// 1 PSI = 6894.757 Па
constexpr uint64_t PSI_TO_PASCAL = 6895u;

constexpr uint64_t MAX_PRESSURE_PSI = 300u;
constexpr uint32_t MAX_PRESSURE_PASCAL =
    MAX_PRESSURE_PSI * PSI_TO_PASCAL; // 2,068,500 Па

// выходной сигнал датчика: 0.5...4.5V
// 0.5V = 0 PSI, 4.5V = 300 PSI
constexpr uint32_t V_MIN_MV = 500u;                 // 0.5V
constexpr uint32_t V_MAX_MV = 4500u;                // 4.5V
...
constexpr uint32_t adcToVoltageMv(int32_t adc_value) noexcept {
...
constexpr uint32_t voltageToPascals(uint32_t voltage_mv) noexcept {
  ...
  uint64_t numerator =
      static_cast<uint64_t>(active_voltage) * MAX_PRESSURE_PSI * PSI_TO_PASCAL;
...
constexpr uint32_t pascalsToDepthMm(uint32_t current_pascals,
...
public:
  void process(int32_t adc_value) {
    adcVoltage_ = PressureConversions::adcToVoltageMv(adc_value);
    if (isFirst) {
      adcPascals_ = atmospheric_pascals_ =
          PressureConversions::voltageToPascals(adcVoltage_);
      isFirst = false;
    } else {
      adcPascals_ = PressureConversions::voltageToPascals(adcVoltage_);
    }
    depthMm_ = PressureConversions::pascalsToDepthMm(adcPascals_,
                                                     atmospheric_pascals_);
  }

  uint32_t getDepthMm() const noexcept { return depthMm_; }
  uint32_t getPressurePascals() const noexcept { return adcPascals_; }
  uint32_t getVoltageADC() const noexcept { return adcVoltage_; }
