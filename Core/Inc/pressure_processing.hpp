...
namespace cc {
template<typename T>
concept AdcDriver = requires(T adc) {
    {
        adc.getADCvalue()
...
template<cc::AdcDriver AdcDriver>
class PressureProcess
{
public:
    PressureProcess(AdcDriver& adcDriver) noexcept
        : _adcDriver{ adcDriver }
    {}

    void process()
    {
        auto adcValue = _adcDriver.getADCvalue();
        if (adcValue.has_value()) {
            adcVoltage_ = adcToVoltageMv(adcValue.value());
...
private:
    AdcDriver& _adcDriver;
    bool dataIsValidate              = true;
    bool isFirst                     = true;
    uint32_t atmospheric_pascals_    = 0;
    uint32_t adcVoltage_             = 0;
    uint32_t adcPascals_             = 0;
    std::optional<uint32_t> depthMm_ = 0;

    // константы преобразований для датчика 300 PSI с внешним 24-битным АЦП

    // VREF = 5V (5000 мВ)
    static constexpr uint32_t VREF_MV = 5000u;

    // максимальное значение 24-битного АЦП (2^23 - 1) с учетом его биполярности
    static constexpr uint32_t ADC_MAX_24BIT = 8388607u;

    // диапазон измерения: 0...300 PSI
    // 1 PSI = 6894.757 Па
    static constexpr uint64_t PSI_TO_PASCAL = 6895u;

    static constexpr uint64_t MAX_PRESSURE_PSI    = 300u;
    static constexpr uint32_t MAX_PRESSURE_PASCAL = MAX_PRESSURE_PSI * PSI_TO_PASCAL; // 2,068,500 Па

    // выходной сигнал датчика: 0.5...4.5V
    // 0.5V = 0 PSI, 4.5V = 300 PSI
    static constexpr uint32_t V_MIN_MV  = 500u;                // 0.5V
    static constexpr uint32_t V_MAX_MV  = 4500u;               // 4.5V
    static constexpr uint32_t V_SPAN_MV = V_MAX_MV - V_MIN_MV; // 4000 мВ

    // перевод показаний АЦП в милливольты (мВ) без учета атмосферного adc_zero
    uint32_t adcToVoltageMv(int32_t adc_value) noexcept
    {
        return static_cast<uint32_t>((static_cast<uint64_t>(adc_value) * VREF_MV) / ADC_MAX_24BIT);
    }
    ...
    uint32_t pascalsToDepthMm(uint32_t current_pascals, uint32_t atmospheric_pascals) noexcept
    {
